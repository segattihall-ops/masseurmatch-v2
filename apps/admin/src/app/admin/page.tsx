...
export default async function AdminPage() {
  ...
  return (
    <main className="mx-auto w-full max-w-5xl px-4 py-8 sm:px-6 sm:py-12">
      <FadeIn>
        <h1 className="text-3xl font-semibold text-ink">Admin Operations</h1>
      ...
      <FadeIn delay={0.1} className="mt-8 flex flex-wrap gap-3">
        <Link href="/moderation" className={buttonVariants()}>
          Moderation queue{metrics.pending > 0 ? ` (${metrics.pending})` : ""}
        </Link>
        <Link href="/verifications" className={buttonVariants({ variant: "outline" })}>
          Identity verifications
        </Link>
        <Link href="/demand-radar" className={buttonVariants({ variant: "outline" })}>
          Demand Radar
        </Link>
      </FadeIn>
