  ...
  return (
    <section className="relative overflow-hidden border-b border-border-subtle bg-bg-subtle">
      <div className="pointer-events-none absolute inset-0" aria-hidden="true">
        ...
        <div className="absolute -bottom-40 left-1/4 h-80 w-80 rounded-full bg-bg-surface opacity-80 blur-3xl" />
      </div>
      ...
      <div className="relative mx-auto grid w-full max-w-6xl gap-12 px-6 pb-16 pt-16 lg:grid-cols-[1.15fr_0.85fr] lg:items-center lg:pb-24 lg:pt-24">
        <div>
          ...
          <h1 className="mt-5 max-w-4xl font-display text-ds-40 font-bold tracking-tight text-text-primary sm:text-ds-56">
          ...
          <form action="/search" method="get" className="mt-8 max-w-2xl" role="search">
            ...
            <div className="flex flex-col gap-3 rounded-3xl border border-border-subtle bg-bg-surface p-3 shadow-sm sm:flex-row">
              <Input
                id="home-search"
                name="q"
                type="search"
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            ...
            <Link
              href="/for-therapists"
              className="inline-flex min-h-11 items-center px-2 text-sm font-semibold text-brand-secondary underline-offset-4 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2"
            >
              List your practice
            </Link>
          </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-1" aria-label="Directory highlights">
          <Card className="overflow-hidden border-border-subtle bg-bg-surface/95">
          ...
          {therapistCount > 0 || cityCount > 0 ? (
            <Card className="border-border-subtle bg-bg-surface/95">
              <CardContent className="grid grid-cols-2 gap-6 p-7 pt-7">
                <div>
                  <p className="font-display text-ds-32 font-bold text-text-primary">
                    {therapistCount}
                  </p>
                  <p className="mt-1 text-sm text-text-secondary">
                    public {therapistCount === 1 ? "profile" : "profiles"}
                  </p>
                </div>
                <div>
                  <p className="font-display text-ds-32 font-bold text-text-primary">{cityCount}</p>
                  <p className="mt-1 text-sm text-text-secondary">
                    active {cityCount === 1 ? "city" : "cities"}
                  </p>
                </div>
              </CardContent>
            </Card>
...
export function FeaturedTherapistsSection({ therapists }: { therapists: TherapistListing[] }) {
  ...
  return (
    <section
      ...
      aria-labelledby="featured-therapists-title"
    >
      <FadeIn
        whileInView
      ...
      >
        <div>
          ...
          <p className="mt-3 max-w-2xl text-text-secondary">
            Discover public profiles from independent therapists currently visible in the
...
export function CityDiscoverySection({ cities }: { cities: CityListing[] }) {
  ...
  return (
    <section
      ...
      aria-labelledby="city-discovery-title"
    >
      <div className="mx-auto w-full max-w-6xl px-6 py-16 sm:py-20">
        ...
        <StaggerList
          whileInView
          as="ul"
          className="mt-9 grid list-none grid-cols-1 gap-4 p-0 sm:grid-cols-2 lg:grid-cols-4"
        >
...
const clientSteps = [
  ...
  [
    "03",
    "Contact directly",
    "Reach out to the independent therapist and arrange details with them directly.",
  ],
] as const;
      ...
      <div className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr] lg:items-start">
        <FadeIn whileInView>
          ...
          <p className="mt-4 max-w-xl leading-7 text-text-secondary">
            MasseurMatch helps people discover independent massage therapists. It does not operate
            therapist businesses, schedule appointments or process client payments.
          </p>
          <Link
            href="/how-it-works"
            className="mt-6 inline-flex text-sm font-semibold text-brand-secondary underline-offset-4 hover:underline"
          >
        ...
        <ol className="grid gap-4 sm:grid-cols-3">
          {clientSteps.map(([number, title, description]) => (
            <li key={number} className="rounded-3xl border border-border-subtle bg-bg-surface p-6">
              <span className="text-xs font-semibold tracking-[0.16em] text-brand-secondary">
                {number}
              </span>
              <h3 className="mt-4 font-display text-ds-18 font-semibold text-text-primary">
                {title}
              </h3>
...
const differentiators = [
  [
    "Rich profiles",
    "See specialties, service categories, location and available profile details before making contact.",
  ],
  [
    "Location discovery",
    "Browse active cities and find therapists using real public directory data.",
  ],
  [
    "Incall and outcall context",
    "Profiles can show whether a therapist offers incall, outcall or both when that information is provided.",
  ],
  [
    "Direct connection",
    "Contact therapists directly rather than being routed through an internal booking marketplace.",
  ],
  [
    "Professional standards",
    "Content and conduct policies set expectations for professional, non-sexual directory use.",
  ],
  [
    "Clear verification language",
    "Identity or profile indicators describe platform checks only and are not professional licensing or certification.",
  ],
] as const;
...
export function DifferentiatorsSection() {
  return (
    <section
      className="border-y border-border-subtle bg-text-primary text-text-inverse"
      aria-labelledby="why-masseurmatch-title"
    >
      <div className="mx-auto w-full max-w-6xl px-6 py-16 sm:py-20">
        <FadeIn whileInView className="max-w-3xl">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-brand-electric">
            Built around useful information
          </p>
          <h2
            id="why-masseurmatch-title"
            className="mt-3 font-display text-ds-32 font-bold tracking-tight sm:text-ds-40"
          >
        ...
        <div className="mt-9 grid gap-px overflow-hidden rounded-3xl border border-white/10 bg-white/10 sm:grid-cols-2 lg:grid-cols-3">
          {differentiators.map(([title, description]) => (
            <article key={title} className="bg-text-primary p-6 sm:p-7">
              ...
              <p className="mt-3 text-sm leading-6 text-text-inverse/75">{description}</p>
            </article>
          ))}
        </div>
      </div>
    ...
    >
      <div className="grid gap-8 lg:grid-cols-[1fr_1.1fr] lg:items-center">
        <FadeIn whileInView>
          ...
          <p className="mt-4 max-w-xl leading-7 text-text-secondary">
            MasseurMatch provides directory tools, profile information and reporting pathways.
            Independent therapists remain responsible for their own services, qualifications,
        ...
        <Card className="border-border-subtle">
          <CardContent className="p-7 pt-7 sm:p-8 sm:pt-8">
            {therapistCount > 0 && cityCount > 0 ? (
              <div className="mb-7 grid grid-cols-2 gap-6 border-b border-border-subtle pb-7">
                <div>
                  ...
                  <p className="mt-1 text-sm text-text-secondary">
                    visible public {therapistCount === 1 ? "profile" : "profiles"}
                  </p>
                </div>
                <div>
                  <p className="font-display text-ds-32 font-bold text-text-primary">{cityCount}</p>
            ...
            <nav aria-label="Trust and safety resources" className="grid gap-3 sm:grid-cols-2">
              <Link
                href="/safety"
                className="rounded-2xl border border-border-subtle p-4 text-sm font-semibold text-text-primary hover:bg-bg-subtle"
              >
                Safety resources
              </Link>
              <Link
...
export function ProviderCtaSection() {
  return (
    <section
      className="mx-auto w-full max-w-6xl px-6 pb-16 sm:pb-20"
      ...
      <div className="overflow-hidden rounded-[2rem] border border-border-subtle bg-brand-soft p-7 sm:p-10 lg:flex lg:items-center lg:justify-between lg:gap-10">
        <div className="max-w-2xl">
          ...
          <p className="mt-4 leading-7 text-text-secondary">
            Create a public listing, present your practice details clearly and make it easier for
            prospective clients to contact you directly. Visibility depends on the directory and
            your profile; results are never guaranteed.
          </p>
        </div>
...
export const homeFaqItems = [
  ...
  [
    ...
    "No. MasseurMatch does not schedule or manage client appointments. Appointment details are arranged directly with the independent therapist.",
  ],
  [
    "Does MasseurMatch process client payments?",
    "No. MasseurMatch does not process payments between clients and therapists. Therapists manage their own business arrangements.",
  ],
  [
    "What does a verification or badge indicator mean?",
    "A MasseurMatch verification or badge indicator refers only to the specific platform check described by the applicable badge policy. It is not professional licensing, certification or a guarantee of service quality.",
  ],
  [
    "Does MasseurMatch verify professional licenses?",
    "MasseurMatch should not be treated as a licensing authority. Visitors are responsible for evaluating a therapist and any credentials relevant to their needs or jurisdiction.",
  ],
  [
    "What are incall and outcall listings?",
    "When a therapist provides this information, incall indicates they may receive clients at their location, while outcall indicates they may travel to a client location. Availability and details are confirmed directly with the therapist.",
  ],
  [
    "How can I report a safety or content concern?",
    "Use the platform reporting and safety resources to report content or conduct concerns for review under MasseurMatch policies.",
  ],
] as const;
  ...
  return (
    ...
    >
      <div className="mx-auto grid w-full max-w-6xl gap-10 px-6 py-16 sm:py-20 lg:grid-cols-[0.7fr_1.3fr]">
        ...
        <div className="divide-y divide-border-subtle rounded-3xl border border-border-subtle bg-bg-surface px-6">
...
export function FinalCtaSection() {
  return (
    <section
      className="mx-auto w-full max-w-6xl px-6 py-16 text-center sm:py-24"
    ...
    >
      ...
      <h2
        id="final-cta-title"
        className="mx-auto mt-3 max-w-3xl font-display text-ds-32 font-bold tracking-tight text-text-primary sm:text-ds-40"
      >
      ...
      <p className="mx-auto mt-4 max-w-2xl leading-7 text-text-secondary">
        Browse the directory, compare public information and contact independent therapists
        directly.
      </p>
